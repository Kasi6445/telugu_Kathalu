...
def generate_redirects(stories_index: list) -> str:
    """Return Cloudflare _redirects file contents for all stories."""
    lines = [
        "# Auto-generated by lib/seo_writer.py — do not edit manually",
        "# Redirects from old ?id= URLs to new clean URLs",
        "",
    ]
    for story in stories_index:
        slug = story.get("slug") or story["id"]
        lines.append(f"/story.html?id={slug}  /story/{slug}/  301")
    lines += [
        "",
